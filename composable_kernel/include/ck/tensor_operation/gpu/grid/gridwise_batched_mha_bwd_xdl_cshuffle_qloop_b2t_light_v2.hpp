...
struct GridwiseBatchedMultiheadAttentionBackward_Qloop_Xdl_CShuffle_Light_V2
{
    ...
    // get_random_4x8() generates 16 random numbers each time
    static constexpr auto DropoutTile = Number<DropoutNThread * 4>{}; // 16
    ...
    __host__ __device__ static constexpr auto GetQBlockDescriptor_K0PerBlock_NPerBlock_K1()
    {
    ...
    {
        // KME change
        // acc_thread_desc_m0_n0_m1_n1_m2_m3_m4_n2 to a_src_thread_desc_k0_m_k1 = {MRepeat, NRepeat, 1, 1, 4, 1, 1, 1}
        // m0_m1_m2_m3 -> k0
        // n0_n1_n2 -> m
        // m4 -> k1
        ...
        return transform_tensor_descriptor(
            ...
            make_tuple(make_merge_transform_v3_division_mod(make_tuple(m0, m1, m2, m3)),    // = {MRepeat, 1, 4, 1}
                       make_merge_transform_v3_division_mod(make_tuple(n0, n1, n2)),        // = {NRepeat, 1, 1}
                       make_pass_through_transform(m4)),                                    // = {1}
    ...
    __host__ __device__ static constexpr auto GetB1BlockDescriptor_BK0PerBlock_NPerBlock_BK1()
    {
        ...
        return make_naive_tensor_descriptor(
            make_tuple(B1K0, Number<Gemm1NPerBlock>{}, B1K1),       // = {4, Gemm1NPerBlock, 8}
            make_tuple(Number<Gemm1NPerBlock + B1BlockLdsExtraN>{} * B1K1, B1K1, I1));
    }
    __host__ __device__ static constexpr auto GetB1BlockDescriptor_BK0PerBlock_NPerBlock_BK1_GemmSequence()
    {
        // B1 matrix in LDS memory, dst of blockwise copy
        constexpr index_t Gemm1NPerBlock_gemmSeq = Gemm1NPerBlock / 4;
        return make_naive_tensor_descriptor(
            make_tuple(B1K0, Number<Gemm1NPerBlock_gemmSeq>{}, B1K1),       // = {4, Gemm1NPerBlock, 8}
            make_tuple(Number<Gemm1NPerBlock_gemmSeq + B1BlockLdsExtraN>{} * B1K1, B1K1, I1));
    }
    ...
    // prepare for dQ by wangaq. 
    template <typename SrcBlockwiseGemm>
    __host__ __device__ static constexpr auto
    MakeLSEGridDescriptor_MB_M0_M1_M2(const LSEGridDesc_M& lse_grid_desc_m)
    {
        const index_t M      = lse_grid_desc_m.GetLength(I0);
        const index_t MBlock = M / NPerBlock;
        constexpr auto SrcBlockDesc_M0_N0_M1_N1_M2_N2_N3_N4 =
            SrcBlockwiseGemm::GetCBlockDescriptor_M0_N0_M1_N1_M2_N2_N3_N4();
        // M0 NXdlPerWave, M1 NWave, M2 num_threads_per_blk
        const auto M0 = SrcBlockDesc_M0_N0_M1_N1_M2_N2_N3_N4.GetLength(I1);
        const auto M1 = SrcBlockDesc_M0_N0_M1_N1_M2_N2_N3_N4.GetLength(I3);
        const auto M2 = SrcBlockDesc_M0_N0_M1_N1_M2_N2_N3_N4.GetLength(I4);

        const auto lse_grid_desc_mb_m0_m1_m2 = transform_tensor_descriptor(
            lse_grid_desc_m,
            make_tuple(make_unmerge_transform(make_tuple(MBlock, M0, M1, M2))),
            make_tuple(Sequence<0>{}),
            make_tuple(Sequence<0, 1, 2, 3>{}));

        return lse_grid_desc_mb_m0_m1_m2;
    }
    ...
    __device__ static auto MakeQGradGridDesc_NBlock_NPerBlock_OBlock_OPerBlock(
        const QGridDesc_K0_M_K1& q_grid_desc_k0_m_k1)
    {
        const auto O0 = q_grid_desc_k0_m_k1.GetLength(I0);
        const auto N  = q_grid_desc_k0_m_k1.GetLength(I1);
        const auto O1 = q_grid_desc_k0_m_k1.GetLength(I2);
        const auto O  = O0 * O1;

        const auto NBlock = N / NPerBlock;
        const auto OBlock = O / Gemm1NPerBlock;

        const auto q_grid_desc_m_o = transform_tensor_descriptor(
            q_grid_desc_k0_m_k1,
            make_tuple(make_pass_through_transform(N),
                       make_merge_transform_v3_division_mod(make_tuple(O0, O1))),
            make_tuple(Sequence<1>{}, Sequence<0, 2>{}),
    ...
    struct GemmBlockwiseCopy
    {
        ...
        // Q matrix in LDS, dst of blockwise copy
        static constexpr auto q_block_desc_k0_m_k1 = GetQBlockDescriptor_K0PerBlock_NPerBlock_K1();
    ...
    // dV / dK / dQ Gemm (type 2 rrr)
    template <typename ASrcThreadDesc_M0_N0_M1_N1_M2_M3_M4_N2,  // decltype(s_blockwise_gemm.GetCThreadDescriptor_M0_N0_M1_N1_M2_M3_M4_N2()) =  {MRepeat, NRepeat, 1, 1, 4, 1, 1, 1}
              typename ASrcBlockDesc_M0_N0_M1_N1_M2_M3_M4_N2>   // decltype(s_blockwise_gemm.GetCBlockDescriptor_M0_N0_M1_N1_M2_M3_M4_N2()) = {MRepeat, NRepeat, MWaves, NWaves, {4, 4, 1}, NPerXDL}
    struct Gemm1
    {
        private:
        ...
        static constexpr auto M3 = ASrcBlockDesc_M0_N0_M1_N1_M2_M3_M4_N2{}.GetLength(I5);   // = 4

        public:
        static constexpr auto AThreadSliceLength_K0 = Number<Gemm1KPerBlock / m4 / M3>{};   // = 32/m4/4 = 8/m4
        static constexpr auto AThreadSliceLength_M  = Number<n0 * n1 * n2>{};               // = NRepeat * 1 * 1 = 1
        static constexpr auto AThreadSliceLength_K1 = Number<m4>{};                         // = m4

        // A source matrix layout in AccVGPR
        static constexpr auto a_src_thread_desc_k0_m_k1 =
            GetA1SrcThreadDescriptor_AK0PerBlock_MPerBlock_AK1(ASrcThreadDesc_M0_N0_M1_N1_M2_M3_M4_N2{});
        ...
        static constexpr auto a_thread_desc_k0_m_k1 = make_naive_tensor_descriptor_packed(
            make_tuple(AThreadSliceLength_K0, AThreadSliceLength_M, AThreadSliceLength_K1));    // = {8/m4, 1, m4}

        // B matrix in LDS memory, dst of blockwise copy
        static constexpr auto b_block_desc_bk0_n_bk1 =
            GetB1BlockDescriptor_BK0PerBlock_NPerBlock_BK1();   // = {4, Gemm1NPerBlock, 8}
        static constexpr auto b_block_desc_bk0_n_bk1_gemmSeq =
            GetB1BlockDescriptor_BK0PerBlock_NPerBlock_BK1_GemmSequence();   // = {4, Gemm1NPerBlock, 8}
        ...
        static constexpr auto a_block_slice_copy_step = make_tuple(AThreadSliceLength_K0, I0, I0);      // = {8, 1, 1}
        static constexpr auto b_block_slice_copy_step = make_multi_index(Gemm1KPerBlock / B1K1, 0, 0);  // = {32, 1, 1}
        ...
        static constexpr index_t GemmKPack = 4;
        ...
        using BlockwiseGemm = BlockwiseGemmXdlops_v2<
            ...
            decltype(MakeGemm1BMmaTileDescriptor_N0_N1_N2_K(b_block_desc_bk0_n_bk1_gemmSeq)),            
            NPerBlock,
            Gemm1NPerBlock,
            Gemm1KPerBlock,
            MPerXdl,
    ...
    struct PGradGemmTile_M_N_O
    {
        ...
        template<class YGradGridDesc_M0_O_M1_>
        __device__ static auto
        MakeYGradGridDescriptor_K0_K1_K2_N0_N1_N2_N3_K3(const YGradGridDesc_M0_O_M1_& ygrad_grid_desc_m0_o_m1)
        {
            const auto M0_ = ygrad_grid_desc_m0_o_m1.GetLength(I0);
            const auto O_  = ygrad_grid_desc_m0_o_m1.GetLength(I1);
            const auto M1_ = ygrad_grid_desc_m0_o_m1.GetLength(I2);

            constexpr auto V_N3 = NPerXdl;
            constexpr auto V_N2 = Gemm0NWaves;
            const auto V_N0     = (M0_ * M1_) / NPerBlock;

            const auto ygrad_grid_desc_m_o = transform_tensor_descriptor(
                ygrad_grid_desc_m0_o_m1,
                make_tuple(make_merge_transform_v3_division_mod(make_tuple(M0_, M1_)),
                    make_pass_through_transform(O_)),
                make_tuple(Sequence<0, 2>{}, Sequence<1>{}),
                make_tuple(Sequence<0>{}, Sequence<1>{}));
        ...
        // __device__ static auto
        // MakeYGradGridDescriptor_K0_K1_K2_N0_N1_N2_N3_K3(const YGridDesc_M_O& y_grid_desc_m_o)
        // {
        //     const auto M_  = v_grid_desc_o0_n_o1.GetLength(I0);

        //     constexpr auto V_N3 = NPerXdl;
        //     constexpr auto V_N2 = Gemm0NWaves;
        //     const auto V_N0     = M_ / NPerBlock;
    ...
    struct D0Operator
    {
        ...
        static constexpr index_t NThreadClusterLengths = 128;
        static_assert(NPerXdl == 16);
        ...
        static constexpr auto d0_thread_desc_ =
            make_naive_tensor_descriptor_packed(make_tuple(I1, I1, I1, I1, D0M2));
        ...
        using D0ThreadwiseCopyLdsToVgpr =
            ThreadwiseTensorSliceTransfer_v4<typename TypeTransform<D0DataType>::Type,   // SrcData
                                             ...
                                             Sequence<1, 1, 1, 1, 4>, // SliceLengths
        ...
        const index_t block_idx_n)
    {
        //dkdv
        {
            ...
            const tensor_operation::element_wise::Scale scale_rp_dropout(s_element_op.Value() * rp_dropout);
            ...
            do
            {
                ...
                // dV = P_drop^T * dY
                {
                    ...
                    // preload data into LDS
                    vgrad_gemm_tile_ygrad_blockwise_copy.RunRead(ygrad_grid_desc_m0_o_m1, ygrad_grid_buf);
                    ...
                    if constexpr(num_gemm1_k_block_inner_loop > 1)
                    {
                        static_for<0, num_gemm1_k_block_inner_loop - 1, 1>{}([&](auto i) {
                            ...
                            vgrad_blockwise_gemm.RunGemmSequence(
                ...
                // dK = scalar * dS^T * Q
                {
                    ...
                    if constexpr(num_gemm1_k_block_inner_loop > 1)
                    {
                        static_for<0, num_gemm1_k_block_inner_loop - 1, 1>{}([&](auto i) {
                            ...
                            kgrad_blockwise_gemm.RunGemmSequence(
            ...
            // shuffle dK&dV and write
            {
                ...
                constexpr auto c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4 =
                    vgrad_blockwise_gemm.GetCThreadDescriptor_M0_N0_M1_N1_M2_N2_N3_N4();    // {MRepeat, MRepeat, 1, 1, 1, 4, 1, 1}

                constexpr auto c_thread_m0 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I0);
                constexpr auto c_thread_n0 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I1);
                constexpr auto c_thread_m1 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I2);
                constexpr auto c_thread_n1 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I3);
                constexpr auto c_thread_m2 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I4);
                constexpr auto c_thread_n2 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I5);
                constexpr auto c_thread_n3 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I6);
                constexpr auto c_thread_n4 = c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4.GetLength(I7);
                ...
                constexpr auto sfc_c_vgpr =
                    SpaceFillingCurve<Sequence<NXdlPerWave, 
                                            Gemm1NXdlPerWave, 
                                            c_thread_m1,
                                            c_thread_n1,
                                            c_thread_m2,
        ...
        // dQ
        {
            ...
            // Vgpr allocation for V
            auto ygrad_thread_buf = generate_tuple(
                [&](auto i) {
                    ignore = i;
                    return StaticBuffer<
                        AddressSpaceEnum::Vgpr,
            ...
            // dY
            const auto ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3 =
                PGradGemmTile_M_N_O::MakeYGradGridDescriptor_K0_K1_K2_N0_N1_N2_N3_K3(ygrad_grid_desc_m0_o_m1);
            ...
            auto gemm_tile_ygrad_blockwise_copy =
                typename GemmBlockwiseCopy::template VBlockwiseCopy<decltype(
                    ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3)>(
                    ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3,
                    make_multi_index(
                        0, 0, wave_m_n_id[I0], block_work_idx_n, 0, wave_id[I1], wave_m_n_id[I1], 0));
            ...
            const index_t num_gemm2_m_block_outer_loop = k_grid_desc_k0_n_k1.GetLength(I1) / MPerBlock;
            // gemm0 M loop start
            index_t gemm2_m_block_outer_index = num_gemm2_m_block_outer_loop - 1;
            ...
            auto s_gemm_tile_k_blockwise_copy =
                typename Gemm3::template ABlockwiseCopy<decltype(k_grid_desc_k0_n_k1)>(
                    k_grid_desc_k0_n_k1,
                    make_multi_index(0,
                                    MPerBlock * gemm2_m_block_outer_index,
                                    0), // will loop over GemmM dimension
                    a_element_op,
                    Gemm3::a_block_desc_ak0_m_ak1,
                    make_multi_index(0, 0, 0),
            ...
            const auto s_gemm_tile_k_block_reset_copy_step =
                make_multi_index(-k_grid_desc_k0_n_k1.GetLength(I0), -MPerBlock, 0);
            const auto s_gemm_tile_k_block_reset_skip_step = make_multi_index(0, -MPerBlock, 0);
            s_blockwise_gemm.SetABlockStartWindow();
            s_blockwise_gemm.SetBBlockStartWindow();

            // prepare for d and lse
            auto lse_grid_desc_mb_m0_m1_m2 =
                MakeLSEGridDescriptor_MB_M0_M1_M2<decltype(s_blockwise_gemm)>(lse_grid_desc_m);

            constexpr auto lse_thread_desc_mb_m0_m1_m2 =
                make_naive_tensor_descriptor_packed(make_tuple(I1, n0, n1, I1));

            auto lse_thread_buf_for_dQ = make_static_buffer<AddressSpaceEnum::Vgpr, FloatLSE>(
                lse_thread_desc_mb_m0_m1_m2.GetElementSpaceSize());

            auto lse_thread_copy_global_to_vgpr_for_dQ =
                ThreadwiseTensorSliceTransfer_v2<FloatLSE,
                                                FloatLSE,
                                                decltype(lse_grid_desc_mb_m0_m1_m2),
                                                decltype(lse_thread_desc_mb_m0_m1_m2),
                                                Sequence<1, n0, n1, I1>,
                                                Sequence<0, 1, 2, 3>,
                                                3,
                                                1,
                                                1,
                                                true /* ResetCoordAfterRun */>{
                                                lse_grid_desc_mb_m0_m1_m2,
                                                make_multi_index(block_work_idx_n, // nblock
                                                                acc0_thread_origin[I1],           // nrepeat
                                                                acc0_thread_origin[I3],           // nwave
                                                                acc0_thread_origin[I7]            // nperxdl
                                                            )};

            auto d_thread_copy_global_to_vgpr_for_dQ =
                ThreadwiseTensorSliceTransfer_v2<FloatD,
                                                FloatGemmAcc,
                                                decltype(lse_grid_desc_mb_m0_m1_m2),
                                                decltype(lse_thread_desc_mb_m0_m1_m2),
            ...
            constexpr auto y_dot_ygrad_thread_desc_mb_m0_m1_m2 =
                lse_thread_desc_mb_m0_m1_m2; // reuse LSE thread descriptor because
                                                // per-thread LSE data and y_dot_ygrad is
                                                // tiled the same way

            auto y_dot_ygrad_thread_buf_for_dQ = make_static_buffer<AddressSpaceEnum::Vgpr, FloatGemmAcc>(
                lse_thread_desc_mb_m0_m1_m2.GetElementSpaceSize());
            
            // prepare for dP^T
            // dP^T: A matrix blockwise copy
            auto pgrad_gemm_tile_v_blockwise_copy =
                typename Gemm0::template ABlockwiseCopy<decltype(v_grid_desc_o0_n_o1)>(
                    v_grid_desc_o0_n_o1,
                    make_multi_index(0,
                                    MPerBlock * gemm2_m_block_outer_index,
                                    0), // will loop over GemmM dimension
            ...
            const auto pgrad_gemm_tile_v_block_reset_copy_step =
                make_multi_index(-v_grid_desc_o0_n_o1.GetLength(I0), -MPerBlock, 0);
            const auto pgrad_gemm_tile_v_block_reset_skip_step = make_multi_index(0, -MPerBlock, 0);
            ...
            auto qgrad_gemm_tile_k_blockwise_copy =
                typename Gemm1::template BBlockwiseCopy<decltype(k_grid_desc_n0_k_n1)>(
                    k_grid_desc_n0_k_n1,
                    make_multi_index(MPerBlock / B1K1 * gemm2_m_block_outer_index, 0, 0),
                    b1_element_op,
                    Gemm1::b_block_desc_bk0_n_bk1, // there n actually is k, k is N, so name can be
                                                // b_block_desc_bn0_k_bn1
                    make_multi_index(0, 0, 0),
            ...
            const auto qgrad_gemm_tile_k_block_next_copy_step =
                make_multi_index(-2 * MPerBlock / B1K1, 0, 0);
                // make_multi_index(-MPerBlock / B1K1, 0, 0);
            const auto qgrad_gemm_tile_k_block_next_skip_step =
                make_multi_index(-MPerBlock / B1K1, 0, 0);
            ...
            // dQ: transform input and output tensor descriptors
            auto qgrad_grid_desc_nblock_nperblock_oblock_operblock =
                MakeQGradGridDesc_NBlock_NPerBlock_OBlock_OPerBlock(q_grid_desc_k0_m_k1);
                // MakeQGradGridDesc_NBlock_NPerBlock_OBlock_OPerBlock(q_grid_desc_m0_k_m1);
            ...
            gemm_tile_q_blockwise_copy.Run(q_grid_desc_k0_m_k1,
                                        q_grid_buf,
                                        GemmBlockwiseCopy::q_block_desc_k0_m_k1,
                                        k_block_buf, // 复用k block buf
                                        I0);
            //
            // load d and lse
            //
            d_thread_copy_global_to_vgpr_for_dQ.Run(lse_grid_desc_mb_m0_m1_m2,
                                            d_grid_buf,
                                            lse_thread_desc_mb_m0_m1_m2,
                                            make_tuple(I0, I0, I0, I0),
                                            y_dot_ygrad_thread_buf_for_dQ);

            lse_thread_copy_global_to_vgpr_for_dQ.Run(lse_grid_desc_mb_m0_m1_m2,
                                                lse_grid_buf,
                                                lse_thread_desc_mb_m0_m1_m2,
                                                make_tuple(I0, I0, I0, I0),
                                                lse_thread_buf_for_dQ);

            // load dY 
            gemm_tile_ygrad_blockwise_copy.SetSrcSliceOrigin(ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3, make_multi_index(
                        0, 0, wave_m_n_id[I0], block_work_idx_n, 0, wave_id[I1], wave_m_n_id[I1], 0));
            static_for<0, GemmBlockwiseCopy::VBlockBufferSize, 1>{}([&](auto ii) {
                gemm_tile_ygrad_blockwise_copy.Run(ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3,
                                            ygrad_grid_buf,
                                            GemmBlockwiseCopy::v_thread_desc_k0_k1_k2_n0_n1_n2_n3_k3,
                                            make_tuple(I0, I0, I0, I0, I0, I0, I0, I0),
                                            ygrad_thread_buf(Number<ii>{}));
                gemm_tile_ygrad_blockwise_copy.MoveSrcSliceWindow(
                    ygrad_grid_desc_k0_k1_k2_n0_n1_n2_n3_k3, GemmBlockwiseCopy::v_block_slice_copy_step);
            ...
            {
                auto m_block_data_idx_on_grid =
                    __builtin_amdgcn_readfirstlane(gemm2_m_block_outer_index * MPerBlock);
                if(c0_matrix_mask.IsTileSkippable(
                    n_block_data_idx_on_grid, m_block_data_idx_on_grid, NPerBlock, MPerBlock))
                {
                    s_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(
                        k_grid_desc_k0_n_k1,
                        s_gemm_tile_k_block_reset_skip_step); // step M
                    pgrad_gemm_tile_v_blockwise_copy.MoveSrcSliceWindow(
                        v_grid_desc_o0_n_o1,
                        pgrad_gemm_tile_v_block_reset_skip_step); // step M
                    qgrad_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(
                        k_grid_desc_n0_k_n1, qgrad_gemm_tile_k_block_next_skip_step);
                    continue;
                }
                ...
                {
                    // preload data into LDS
                    s_gemm_tile_k_blockwise_copy.RunRead(k_grid_desc_k0_n_k1, k_grid_buf);

                    s_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(k_grid_desc_k0_n_k1,
                                                                    Gemm3::a_block_slice_copy_step);
                    ...
                    s_gemm_tile_k_blockwise_copy.RunWrite(Gemm3::a_block_desc_ak0_m_ak1,
                                                        gemm3_a_block_buf);
                    ...
                    if constexpr(HasMainKBlockLoop)
                    {
                        ...
                        do
                        {
                            ...
                            s_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(
                                k_grid_desc_k0_n_k1, Gemm3::a_block_slice_copy_step);
                ...
                if constexpr(MaskOutUpperTriangle || PadN)
                {
                    static_for<0, Acc0TileIterator::GetNumOfAccess(), 1>{}([&](auto i) {
                        ...
                        auto m_global    = m_local + m_block_data_idx_on_grid;
                        auto n_global    = n_local + n_block_data_idx_on_grid;
                        bool masked_flag = c0_matrix_mask.IsMaskedElement(n_global, m_global);
                        s_element_op(s_slash_p_thread_buf(i),
                                    masked_flag ? -ck::NumericLimits<float>::Infinity()
                                                : s_slash_p_thread_buf[i]);
                        // S --> P
                        s_slash_p_thread_buf(i) = false && ck::math::isnan(s_slash_p_thread_buf[i])
                                        ? 0
                                        : math::exp(s_slash_p_thread_buf[i] - lse_thread_buf_for_dQ[Number<0>{}]); 
                    });
                }
                else
                {
                    static_for<0, s_slash_p_thread_buf.Size(), 1>{}([&](auto i) {
                        s_element_op(s_slash_p_thread_buf(i), s_slash_p_thread_buf[i]);
                        // S --> P
                        s_slash_p_thread_buf(i) = ck::math::isnan(s_slash_p_thread_buf[i])
                                        ? 0
                                        : math::exp(s_slash_p_thread_buf[i] - lse_thread_buf_for_dQ[Number<0>{}]); 
                    });
                }
                if constexpr(IsDropout)
                {
                    ...
                    // P_dropped
                    blockwise_dropout.template ApplyDropoutAttnBwdQ<decltype(s_slash_p_thread_buf),
                                                                   decltype(DropoutTile),
                                                                   true>(
                        s_slash_p_thread_buf, ph, global_elem_id, raw_n_padded);
                }
                ...
                {
                    // preload data into LDS
                    pgrad_gemm_tile_v_blockwise_copy.RunRead(v_grid_desc_o0_n_o1,
                                                                v_grid_buf);

                    pgrad_gemm_tile_v_blockwise_copy.MoveSrcSliceWindow(
                        v_grid_desc_o0_n_o1, Gemm0::a_block_slice_copy_step);

                    block_sync_lds(); // wait for previous LDS read

                    pgrad_thread_buf.Clear();

                    pgrad_gemm_tile_v_blockwise_copy.RunWrite(Gemm0::a_block_desc_ak0_m_ak1,
                                                                gemm0_a_block_buf);
                    ...
                    if constexpr(num_ok_block_main_loop > 1)
                    {
                        static_for<0, num_ok_block_main_loop - 1, 1>{}([&](auto i) {
                            ...
                            block_sync_lds();
                            // wangaq debug
                            // debug::print_shared<ck::half_t, 1, 16>(gemm0_a_block_buf.p_data_, index_t(Gemm0::a_block_desc_ak0_m_ak1.GetElementSpaceSize()));

                            pgrad_blockwise_gemm.Run(
                                gemm0_a_block_buf, ygrad_thread_buf(Number<i>{}), pgrad_thread_buf);
                    ...
                    // tail
                    {
                        ...
                        pgrad_blockwise_gemm.Run(gemm0_a_block_buf,
                                                ygrad_thread_buf(Number<num_ok_block_main_loop - 1>{}),
                                                pgrad_thread_buf);
                    }

                } // end gemm dP

                // dS^T = P^T * (dP^T - Y_dot_dY)
                auto& sgrad_thread_buf_for_dQ = pgrad_thread_buf;
                static_for<0, pgrad_thread_buf.Size(), 1>{}([&](auto i) {
                    // dS and P has same thread buf layout
                    bool undropped_flag = s_slash_p_thread_buf[i] >= 0;
                    sgrad_thread_buf_for_dQ(i) =
                        s_slash_p_thread_buf[i] *
                        (undropped_flag ? (pgrad_thread_buf[i] - y_dot_ygrad_thread_buf_for_dQ[Number<0>{}])
                                        : y_dot_ygrad_thread_buf_for_dQ[Number<0>{}]);
                                        // : y_dot_ygrad_thread_buf_for_dQ[Number<0>{}]) / 2072385.0; // 2072385.0 for test by wangaq debug
                });
                ...
                // dQ^T = K^T * dS^T
                {
                    ...
                    if constexpr(num_gemm1_k_block_inner_loop > 1)
                    {
                        static_for<0, num_gemm1_k_block_inner_loop - 1, 1>{}([&](auto i) {
                            ...
                            qgrad_blockwise_gemm.RunGemmSequence(
                                gemm1_a_thread_buf, gemm1_b_block_buf, qgrad_thread_buf);
                            // qgrad_blockwise_gemm.RunGemmSequenceMRepeatOne(
                            //     gemm1_a_thread_buf, gemm1_b_block_buf, qgrad_thread_buf);
                ...
                s_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(
                    k_grid_desc_k0_n_k1,
                    s_gemm_tile_k_block_reset_copy_step); // rewind K and step M
                s_blockwise_gemm.MoveBBlockSrcSliceWindow(Gemm3::b_block_reset_copy_step);
                pgrad_gemm_tile_v_blockwise_copy.MoveSrcSliceWindow(
                    v_grid_desc_o0_n_o1,
                    pgrad_gemm_tile_v_block_reset_copy_step); // rewind O and step M

                qgrad_gemm_tile_k_blockwise_copy.MoveSrcSliceWindow(
                    k_grid_desc_n0_k_n1, qgrad_gemm_tile_k_block_next_copy_step);
            } while (0 < gemm2_m_block_outer_index--);
            ...
            // shuffle dQ and write
            {
                ...
                // thread desc same with kgrad_blockwise_gemm
                constexpr auto c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4 =
                    qgrad_blockwise_gemm.GetCThreadDescriptor_M0_N0_M1_N1_M2_N2_N3_N4();    // {MRepeat, MRepeat, 1, 1, 1, 4, 1, 1}
                ...
                // shuffle: threadwise copy C from VGPR to LDS
                auto qgrad_thread_copy_vgpr_to_lds =
                    ThreadwiseTensorSliceTransfer_v1r3<FloatGemmAcc,
                                                    OutputDataType,
                                                    decltype(c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4),
                                                    decltype(c_block_desc_m0_n0_m1_n1_m2_n2_n3_n4),
                ...
                // shuffle: blockwise copy C from LDS to global
                auto qgrad_shuffle_block_copy_lds_to_global = ThreadGroupTensorSliceTransfer_v6r1<
                    ThisThreadBlock,            // ThreadGroup
                    CElementwiseOperation,      // ElementwiseOperation,
                    CGlobalMemoryDataOperation, // DstInMemOp,
                    Sequence<1,
                    ...
                    OutputDataType,       // typename DstData,
                    decltype(c_shuffle_block_desc_mblock_mperblock_nblock_nperblock),
                    decltype(qgrad_grid_desc_nblock_nperblock_oblock_operblock),
                    Sequence<0, 1, 2, 3>,                           // typename DimAccessOrder,
                    3,                                              // index_t VectorDim,
                    CShuffleBlockTransferScalarPerVector_NPerBlock, // index_t ScalarPerVector,
                    true,  // bool ThreadTransferSrcResetCoordinateAfterRun,
                    ...
                    {c_shuffle_block_desc_mblock_mperblock_nblock_nperblock,
                    make_multi_index(0, 0, 0, 0),
                    qgrad_grid_desc_nblock_nperblock_oblock_operblock,
                    make_multi_index(block_work_idx_n, 0, block_work_idx[I1], 0),
                    c_element_op};
                ...
                static_for<0, num_access, 1>{}([&](auto access_id) {
                    ...
                    // each thread write its data from VGPR to LDS
                    qgrad_thread_copy_vgpr_to_lds.Run(c_thread_desc_m0_n0_m1_n1_m2_n2_n3_n4,
                                                    sfc_c_vgpr.GetIndexTupleOfNumber(access_id),
                                                    qgrad_thread_buf,
                                                    c_block_desc_m0_n0_m1_n1_m2_n2_n3_n4,
                                                    c_shuffle_block_buf);
                    ...
                    // each block copy its data from LDS to global
                    qgrad_shuffle_block_copy_lds_to_global.Run(
                        c_shuffle_block_desc_mblock_mperblock_nblock_nperblock,
                        c_shuffle_block_buf,
                        qgrad_grid_desc_nblock_nperblock_oblock_operblock,
                        qgrad_grid_buf);
                    ...
                    if constexpr(access_id < num_access - 1)
                    {
                        ...
                        // move on C
                        qgrad_shuffle_block_copy_lds_to_global.MoveDstSliceWindow(
                            qgrad_grid_desc_nblock_nperblock_oblock_operblock, c_global_step);
                    }
                });
            }
